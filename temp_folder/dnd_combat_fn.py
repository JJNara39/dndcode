...
def make_team(player, monsters_data, npc_data, team_controller):
    contestant_team = []
    if player:
        for cr in select_combatants(player[:], monsters_data, npc_data):
            cr_copy = copy.deepcopy(cr)
            cr_copy.original_name = cr_copy.name
            cr_copy.name += f" from {team_controller}'s Team"
            contestant_team.append(cr_copy)
    else:
        for cr in select_combatants(None, monsters_data, npc_data):
            cr_copy = copy.deepcopy(cr)
            cr_copy.original_name = cr_copy.name
            cr_copy.name += f" from {team_controller}'s Team"
            contestant_team.append(cr_copy)   
...
def format_damage_string(act_data):
    dmg = act_data.get('Damage', '')
    dmgtype = act_data.get('Damage Type', '')
    dmgcomp = act_data.get('Damage Components', '')
    if dmg and dmgtype:
        return f"{dmg} {dmgtype}"
    elif dmgcomp:
        parts = []

        for comp in dmgcomp:
            dice = comp.get("Dice", "")
            modifier = comp.get("Modifier", "")
            dmg_type = comp.get("Type", "")
            dmg_str = dice
            if modifier:
                dmg_str += f"+{modifier}"
            dmg_str += f" {dmg_type}"
...
def hit(target, attacker, attack, attack_roll, announcer, current_turn): #This is still doing too much, in that it builds dmg, rolls crit dmg, prints narration, applies damage, and processes death
    #but for now it will do, we will revisit later
    parts = []
    total = 0
    combat_damage = []
    damage_components = attack.get("Damage Components")
    for i, comp in enumerate(damage_components):
        ...
        if "Swarm of Venomous Snakes" in attacker.name:
            if any(c.get("Name") == "Bloodied" for c in attacker.conditions):
                if i == 0:
                    dice_str = attack['Bloodied Damage']
        ...
        if attack_roll == 20:
            ...
            dmg = roll(dice_str)
        if dice_mod == "0":
            parts.append(f"{parts_dice_str} {damage_type}")
        else:
            parts.append(f"{dice_str} {damage_type}")
        combat_damage.append((dmg, damage_type))
        parts_str = " + ".join(pt for pt in parts)
    if "Active Effects" in attack:
        apply_weapon_condition(attacker, attack, target, current_turn)                
    total = target.take_damage(combat_damage)                            
    print(f"{attacker.name} crits {target.name} for {parts_str} or {total} damage!")                                     
    target.process_damage(total, announcer)
...
def resolve_attack_roll(attacker, target, attack_data, current_turn, announcer):
    hit_mod = attack_data.get('Attack Bonus', 0)

    attack_roll = attacker.roll_attack(target, attack_data, announcer)
    ...
    if attack_roll == 1:
        ...
        hit(target, attacker, attack_data, attack_roll, announcer, current_turn)
...
def weapon_attack(attacker, atk_key, target, announcer, current_turn, home_team, opponent_team, init_tracker, attack_data = None):   
    if isinstance(attacker, Player):
        if attack_data is None:
            attack_data = attacker.attacksspellcasting[atk_key]
        resolve_attack_roll(attacker, target, attack_data, current_turn, announcer)
        ...
        if atk_key == "Multiattack":            
            ...
            for sub_action in sequence:  # List like ["Bite", "Claw"]
                #First, check if the target is still valid, this is what my previous validity check looked like
                ...
                if special_skip(sub_action, attacker, target):
                    continue
                attack_data = attacker.actions["Multiattack"]["Attacks"][sub_action]                
                if attack_data['Attack Type'] == "Attack Roll":
                    resolve_attack_roll(attacker, target, attack_data, current_turn, announcer)   
                elif attack_data['Attack Type'] == "Saving Throw":
                    resolve_saving_throw(attacker, target, attack_data, announcer, current_turn)         
        else:
...
def resolve_saving_throw(attacker, target, attack_data, announcer, current_turn):
    
    save_type = attack_data.get("Save Type")
    save_dc = attack_data.get("Save DC")

    roll_total = target.roll_saving_throw(save_type)
    
    print(f"{target.name} rolls {roll_total} {save_type} save vs DC {save_dc}")
    if roll_total >= save_dc:
        print(f"{target.name} succeeds the save!")

        if attack_data.get("Half Damage on Save"):
            apply_damage(target, announcer, attack_data, attacker, half=True)
    
    else:
        print(f"{target.name} fails the save!")

        apply_damage(target, announcer, attack_data, attacker)
    
    process_effects(attack_data, attacker, target, current_turn)

def special_skip(attack_name, attacker, target):
    # Trex and Giant Crocodile special cases: skip Tail if target is Grappled by this creature
    if attack_name == "Tail":
        if "Giant Crocodile" in attacker.name or "Tyrannosaurus Rex" in attacker.name:
            if any(
                c.get("Name", "").startswith("Grappled") 
                and c.get("Source") == attacker
                for c in target.conditions
            ):
    ...
    while available_actions or available_movement:
        ...
        for idx, (label, act) in enumerate(options, 1):
            ...
            if act in available_actions and act != "Multiattack":        
                ...
                if atk_ogname in dnd_spells.spells:
                    ...
                    display_text = f"{act_data['Name']} ({'+' if act_data.get('Attack Bonus', 0)>=0 else ''}{act_data.get('Attack Bonus', 0)} to hit) | {format_damage_string(act_data)} {proficient}"
                elif verse == " (2H)":
                    ...
                    if throw_act:
                        if act_data.get('Properties', {}).get('Type', '') == "Thrown":
                            display_text = f"(Throw) {act_data['Name']} ({'+' if act_data.get('Attack Bonus', 0)>=0 else ''}{act_data.get('Attack Bonus', 0)} to hit) | {format_damage_string(act_data)} {proficient}" #There is more to throwing a weapon than this...
                        else:
        ...
        if label == "Action": 
            ...
            if atk_type is not None:
                ...
                elif "Weapon" or "Attack Roll" in action_data.get("Attack Type"):
                    if "Weapon" in action_data.get("Attack Type"):
...
def select_combatants(available_players, monster_data, npc_data):
    ...
    while True:
        if available_players:
            ...
            choice_type = input("\nAdd a (M) - Monster, (N) - NPC, or (D) - Done selecting? ").lower()
        ...
        if choice_type == 'd':
        ...
        elif choice_type == 'p' and available_players:
            ...
            while True:
                filtered_keys = select_cr(data, label)

                if filtered_keys == "Back":
                    break
                base_entity = select_creature(data, label, filtered_keys)
                if base_entity == "Different CR":
                    continue
                elif base_entity == "Selection":
                    break
                new_entities = select_quantity(base_entity)     
                contestants.extend(new_entities)
                break
...
def select_cr(data, label):
    ...
    while True:
        ...
        for idx, cr in enumerate(cr_list, 1):
            print(f"{idx} - CR {cr}")
        print(f"{idx+1} - Return to Selection")
        ...
        try:
            ...
            elif cr_choice == len(cr_list) + 1:
                filtered_keys = "Back"
            else:
                print("Invalid CR selection.")
                continue #continue repeats the while loop
        ...
        break #break leaves the loop, and this can only be reached if none of the "continue" lines are met
    
    return filtered_keys

def select_creature(data, label, filtered_keys):
    while True:
        base_entity = None
        print(f"\nAvailable {label}s:")
        print("0 - Random")
        ...
        print(f"{idx+1} - Different CR")
        print(f"{idx+2} - Back to Selection")
        ...
        try:
            ...
            if entity_choice == 0:
                ...
                base_entity = Monster(data[selected_key])
            #Here! I need to add a go back option        
            elif entity_choice == len(filtered_keys) + 1:
                base_entity = "Different CR"
            elif entity_choice == len(filtered_keys) + 2:
                base_entity = "Selection"
...
def select_quantity(base_entity):
    new_entities = []
    while True:
        try:
