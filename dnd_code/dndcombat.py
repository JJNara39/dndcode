...
def combat(player):
    ...
    print("Who is controlling Team 2? Enter name:")
    #print("Who is controlling Team 2? Enter name or 'AI':")
    team2_controller = input("> ").strip()
    print(f"Choose team for {team2_controller}:")
    contestant2_team = dnd_combat_fn.make_team(player, monsters_data, npc_data, team2_controller)
    ...
    while True:
        ...
        #if dnd_combat_fn.controller_of(attacker) == "AI": #Commenting this out for not since AI is not a thing in this yet
        #    dnd_combat_fn.run_ai_turn(attacker)
        #else: #Tab the rest of this when you bring that if-check
        has_acted_this_round[attacker.name] = True
        print(f"{attacker.name}'s turn!")
        print(f"{attacker.name} is currently at {attacker.currenthitpoints} hit point(s).")
        ...
        if dnd_combat_fn.check_combat_end(player, announcer, contestant1_team):
            ...
            break          #This ends the Tabbing mentioned above from the AI 
        # End of round check
