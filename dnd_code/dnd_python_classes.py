...
import dnd_code.dnd_class_gen as dnd_class_gen
import dnd_code.dnd_race_gen as dnd_race_gen
import dnd_code.dnd_tools as dnd_tools
import dnd_code.dnd_spells as dnd_spells
import dnd_code.dnd_update as dnd_update
...
class Player:
    ...
    def take_damage(self, combat_damage):
        total = 0
        for dmg, type in combat_damage:
    ...
    def process_damage(self, amount, announcer):
    ...
    def check_advantage(self, target, attack_data):
        ...
        if self.advantage_against.get(target.name, False):
            advantage_sources += 1

        if self.disadvantage_against.get(target.name, False):
            disadvantage_sources += 1

        # ---- Special attack/creature logic ----
        if self.has_special_advantage(target, attack_data):
            advantage_sources += 1

        if self.has_special_disadvantage(target, attack_data):
            disadvantage_sources += 1

        # ---- Final resolution ----
        #On page 12 of the 2024 PHB, it is stated that adv/disadv do not stack, so there for any amount of adv cancel out any amount of disadv
        if advantage_sources > 0 and disadvantage_sources == 0:
            return "Advantage"

        if disadvantage_sources > 0 and advantage_sources == 0:
            return "Disadvantage"

        return None
    
    def has_special_advantage(self, target, attack_data):
        if "Blood Frenzy" in self.traits:
            if target.currenthitpoints < target.hitpoints:
                return True
        
        return False

    def has_special_disadvantage(self, target, attack_data):
        return

    def roll_attack(self, target, attack_data, announcer):
        roll1 = dice(20)
        roll2 = dice(20)

        adv_state = self.check_advantage(target, attack_data)

        if adv_state == "Advantage":
            announcer.announce_advantage(self.name)
            return max(roll1, roll2)

        elif adv_state == "Disadvantage":
            announcer.announce_disadvantage(self.name)
            return min(roll1, roll2)

        else:
            return roll1       

    def roll_saving_throw(self, save_type):
        ability_mod = math.floor((self.ability_scores[save_type] - 10) / 2)

        prof = self.profbonus if f"{save_type} ST" in self.saving_throws else 0
        bonus = self.saving_throws.get(f"{save_type} ST", 0)

        roll_total = dice(20) + ability_mod + max(prof, bonus)
        return roll_total
    ...
    def has_condition(self, name):
        return (c.get("Name") == name for c in self.conditions)

    def is_invalid_target(self):
        return self.is_down() or self.has_condition("Unconscious")
...
class Monster:
    ...
    def process_damage(self, amount, announcer):
        self.currenthitpoints -= amount
        if self.currenthitpoints <= 0:
            self.currenthitpoints = 0 #I do not know right now if this is how I wanna treat deaths. I like that if currenthitpoints = -max_hp you are dead, but let this play out for now...
...
class Announcer:
    ...
    def announce_advantage(): #The announce_ Advantage and Disadvantage functions are moot, when called they should be announced like other announcements, include sections for Adv and Disadv
        return
    
    def announce_disadvantage():
        return    